...
def training(opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    scene = SceneEG3D(eg3d_model=eg3d_model, gaussians=gaussians, overfit_single_id=opt.overfit_single_id, flame_init=False)
    ...
    if opt.pre_offset:
        decoder_params.append({'params': eg3d_model.pre_offset_decoder.parameters(), 'lr': 0, "name": "pre_offset_decoder"})
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        flame_xyz = gaussians.get_xyz
        # get decoded features based on new xyz
        viewpoint, gt_image, decoded_features = scene.get_camera_and_target(fov_deg, xyz=flame_xyz, z=scene.z)

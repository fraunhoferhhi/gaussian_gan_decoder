...
class GaussianConv(torch.nn.Module):
    def __init__(
        ...
        xyz,
        input_channel=256,
        layers_channel=[256, 128, 64, 32, 3],
        downsample_layer=[],
        upsample_layer=[],
        K=8,
    ):
    ...
    def init_kmeans_knn(self, xyz, len_sample_layer):
        ...
        for i in range(len_sample_layer):
            ...
            down_centroids = torch_scatter.scatter(
                last_xyz, self.kmeans_labels[-1], dim=0, reduce="mean"
            )  # [cluster_num=N//5, D]
    ...
    def init_conv_params(self, input_channel, layers_channel):
        ...
        for out_channel in layers_channel:
            self.kernels.append(
                torch.randn(out_channel, self.K * input_channel) * 0.1
            )  # [out_channel, K*input_channel]

...
def training(opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    path = "./output"
    wandb.init(project='3DGaussianHeads', dir="./output", group="flame-init", name="classic-preoffset")
   
    white_background = False
    first_iter = 0
    gaussians = GaussianModel(3)
    eg3d_model = GaussianTriplaneDecoder(num_gaussians_per_axis=65, triplane_generator_ckp="../eg3d/eg3d/networks/var3-128.pkl", pre_offset=opt.xyz_mlp)
    scene = SceneEG3D(eg3d_model=eg3d_model, gaussians=gaussians, overfit_single_id=opt.overfit_single_id, flame_init=True)
    
    decoder_params = None
    if opt.xyz_mlp:
        decoder_params = [{'params': eg3d_model.pre_offset_decoder.parameters(), 'lr': opt.rotation_lr, "name": "pre_offset_decoder"}]
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        fov_deg = np.random.rand() * 5 + 13 if iteration != 1 else 17
        extrinsic = None if iteration != 1 else scene.get_random_extrinsic(0, 0)
        viewpoint_cam, gt_image, decoded_features = scene.get_camera_and_target(fov_deg, extrinsic=extrinsic)
        ...
        xyz_offset = decoded_features["_pre_offset"].squeeze(0) if opt.xyz_mlp else None
        render_pkg = render_simple(viewpoint_cam, gaussians, bg_color=bg, xyz_offset=xyz_offset)
        ...
        with torch.no_grad():
            if iteration % 500 == 0 or iteration == 1:
                # mask = (image > 0)*1.
                # blended_image = mask*image + (1 - mask)*gt_image
                # blended_image = blended_image.detach().cpu().numpy().transpose(1, 2, 0)
                # wandb.log({"overlay": [wandb.Image(blended_image, caption="overlay")]}, step=iteration)

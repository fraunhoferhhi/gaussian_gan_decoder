...
class CustomCam:
    def __init__(self, size, fov, extr, znear=0.01, zfar=10.0):
        ...
        self.znear = znear
        self.zfar = zfar
        # invert and transpose for conversion between eg3d and 3dgs

        self.world_view_transform = extr.T.inverse()

        self.projection_matrix = getProjectionMatrix(znear=self.znear, zfar=self.zfar, fovX=self.FoVx, fovY=self.FoVy).transpose(0,1).cuda()

        # self.projection_matrix = getProjectionMatrixCustom(focal_length, focal_length, width, height, 0, 0).transpose(0,1).cuda()

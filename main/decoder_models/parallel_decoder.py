...
class ParallelDecoder(nn.Module):
    def __init__(self, G, hidden_dim=128, use_xyz_embedding=True, use_gen_finetune=True, triplane_sr="None", device="cuda"):
        ...
        self.scale_decoder = Decoder(n_features=32 + position_dim, out_features=3, hidden_dim=hidden_dim).to(device)
        self.rotation_decoder = Decoder(n_features=32 + position_dim, out_features=4, hidden_dim=hidden_dim).to(device)
        self.opacity_decoder = Decoder(n_features=32 + position_dim, out_features=1, hidden_dim=hidden_dim).to(device)
        self.color_decoder = Decoder(n_features=32 + position_dim, out_features=3, hidden_dim=hidden_dim).to(device)
    ...
    def forward(self, z, gan_camera_params, init_position, truncation_psi):
        ...
        synth = self.G.synthesis(ws, gan_camera_params, noise_mode="const")
        ...
        if self.use_xyz_embedding:
            pos = self.embedder(init_position)
        else:
            pos = init_position

        xyz = self.xyz_decoder(plane_features, pos) * 0.01 + init_position
        result.xyz = xyz

        scale = self.activate_scale(self.scale_decoder(plane_features, pos))
        result.scale = scale

        rotation = self.rotation_decoder(plane_features, pos)
        result.rotation = rotation

        opacity = self.opacity_decoder(plane_features, pos)
        result.opacity = opacity

        color = self.color_decoder(plane_features, pos)
        result.color = color
        return result

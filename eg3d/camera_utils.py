...
class LookAtPoseSampler:
    ...
    @staticmethod
    def sample(horizontal_mean, vertical_mean, horizontal_stddev=0, vertical_stddev=0, radius=1, batch_size=1, device='cpu', return_hv=False):
        h = torch.randn((batch_size, 1), device=device) * horizontal_stddev + horizontal_mean
        v = torch.randn((batch_size, 1), device=device) * vertical_stddev + vertical_mean
        v = torch.clamp(v, 1e-5, math.pi - 1e-5)
        ...
        if return_hv:
            return create_cam2world_matrix(forward_vectors, camera_origins), h, v
        return create_cam2world_matrix(forward_vectors, camera_origins)
...
class UniformCameraPoseSampler:
    ...
    @staticmethod
    def sample(horizontal_mean=math.pi/2, vertical_mean=math.pi/2, horizontal_stddev=0, vertical_stddev=0, radius=1, batch_size=1, device='cpu', return_hv=False):

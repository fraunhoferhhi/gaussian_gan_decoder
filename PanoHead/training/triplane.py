...
class TriPlaneGenerator(torch.nn.Module):
    def __init__(
        ...
        **synthesis_kwargs,  # Arguments for SynthesisNetwork.
        ...
        self.bcg_synthesis = (
            SynthesisNetwork(
                w_dim, img_resolution=self.superresolution.input_resolution, img_channels=32, **bcg_synthesis_kwargs
            )
        ...
        self.t_mapping = MappingNetwork(
            z_dim=z_dim, c_dim=c_dim, w_dim=3, num_ws=1, last_activation="linear", lr_multiplier=1.0, **mapping_kwargs
        )
        self.neural_rendering_resolution = 64
        self.rendering_kwargs = rendering_kwargs

        self._last_planes = None
        self.without_sr = without_sr
    ...
    def synthesis(
        ...
        **synthesis_kwargs,
        ...
        planes = planes.view(
            len(planes), 3, 32 * self.rendering_kwargs["triplane_depth"], planes.shape[-2], planes.shape[-1]
        )

        # Perform volume rendering
        feature_samples, depth_samples, weights_samples = self.renderer(
            planes, self.decoder, ray_origins, ray_directions, self.rendering_kwargs
        )  # channels last
        ...
        if self.bcg_synthesis:
            ...
            bcg_image = -1 * torch.ones_like(bcg_image)

...
class SequentialDecoderReverse(nn.Module):
    def __init__(self, G, hidden_dim=128, use_xyz_embedding=True, use_gen_finetune=True, triplane_sr="None", device="cuda"):
        ...
        self.opacity_decoder = Decoder(n_features=32 + position_dim + 3, out_features=1, hidden_dim=hidden_dim).to(device)
        self.rotation_decoder = Decoder(n_features=32 + position_dim + 4, out_features=4, hidden_dim=hidden_dim).to(device)
        self.scale_decoder = Decoder(n_features=32 + position_dim + 8, out_features=3, hidden_dim=hidden_dim).to(device)
        self.xyz_decoder = Decoder(n_features=32 + position_dim + 11, out_features=3, hidden_dim=hidden_dim).to(device)
    ...
    def forward(self, z, gan_camera_params, init_position, truncation_psi):
        ...
        result.color = color
        current_info = torch.concat([current_info, color], dim=-1)
